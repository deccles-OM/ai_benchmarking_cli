...
from ford_data_analyst_benchmark.evaluation.scorer import BenchmarkScorer
...
def generate_markdown_report(results_summary, response_path, total_score, max_score, avg_score):
    ...
    # Find weaknesses - categories that are NOT the best performers
    # Show only the weakest performing categories, not those that are relatively strong
    worst_categories = sorted_categories[-2:]  # Bottom 2 categories
    for category, scores in worst_categories:
        if category != sorted_categories[0][0]:  # Don't include the best category
    ...
    report.append("")
    if variance > 40:
        report.append("1. **Address Inconsistency**: The large performance gap suggests variable capabilities. Focus on consistent improvement across all domains.")
    
    if percentage < 50:
        report.append("2. **Foundational Skills**: Review core competencies and fundamental task understanding.")
    elif percentage < 65:
        report.append("2. **Bridge Performance Gaps**: Implement targeted training for weaker categories.")
    else:
        report.append("2. **Maintain Momentum**: Continue improving weak areas while sustaining strengths.")
    
    report.append("3. **Iterative Testing**: Re-evaluate regularly to track improvement progress.")
    report.append("")
...
def evaluate_responses(response_file_path: str, benchmark_dir: str = "ford_data_analyst_benchmark"):
    """
    ...
    Args:
        ...
        benchmark_dir: Path to the benchmark directory containing tasks and datasets
    """
    ...
    max_score = 0
    results = []
    results_by_category = defaultdict(lambda: {'score': 0, 'max_points': 0, 'count': 0})
    ...
    for i, response_item in enumerate(responses, 1):
        ...
        try:
            print(f"{status} Task {i}/{len(responses)}: {task_key} [{category}] {task_id}")
            print(f"   Score: {combined_score:.2%} (Accuracy: {accuracy:.2%}, Reasoning: {reasoning:.2%})")
            question_preview = str(pretty_question)[:80]
            response_preview = str(pretty_response)[:100]
            print(f"   Question: {question_preview}...")
        except UnicodeEncodeError:
            # Skip detailed output if encoding issues, just show task status
    ...
    try:
        from generate_answer_comparison import generate_answer_comparison_table
        generate_answer_comparison_table(str(response_path), benchmark_dir)
...
if __name__ == "__main__":
    ...
    evaluate_responses(response_file)
